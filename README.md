...
Render molecular orbitals from Gaussian cube files (`.cube`). Requires the `--mo` flag. The cube file contains both the molecular geometry and a 3D volumetric grid of orbital values — no separate XYZ file needed.
...
| `--mo-colors POS NEG` | Lobe colors as hex (default: `#2554A5` `#851639`) |

Cube files are typically generated by ORCA (`orca_plot` block) or Gaussian (`cubegen`). are projected onto 2D and drawn as SVG paths behind and in front of the molecule. Each 3D lobe is tracked separately so lobes stay coherent during rotation GIFs.
...
| `--mo-colors` | MO lobe colors (two hex values: POS NEG) |
